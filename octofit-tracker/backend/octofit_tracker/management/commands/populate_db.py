...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Create workouts
        Workout.objects.create(name='Morning Cardio', description='Cardio for all', duration=40)
        Workout.objects.create(name='Strength Training', description='Strength for all', duration=50)

        # Create leaderboard
        Leaderboard.objects.create(user=tony, points=1000)
        Leaderboard.objects.create(user=steve, points=900)
        Leaderboard.objects.create(user=bruce, points=1100)
